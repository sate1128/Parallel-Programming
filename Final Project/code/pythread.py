...
class myThread (threading.Thread):
    def __init__(self, local_idf, local_tf, part, s, e):
        threading.Thread.__init__(self)
        self.local_idf = local_idf
        self.local_tf = local_tf
        self.part = part
        self.s = s
        self.e = e
    def run(self):
        idf_func(self.local_idf,self.part)
        tf_func(self.local_tf,self.s,self.e)

def idf_func(local_idf,part):
...
thread1 = myThread(local1_idf,local1_tf,0,0,3)
thread2 = myThread(local2_idf,local2_tf,1,3,6)
thread3 = myThread(local3_idf,local3_tf,2,6,8)
thread4 = myThread(local4_idf,local4_tf,3,8,10)
...
#idf = local1_idf + local2_idf
idf = local1_idf + local2_idf + local3_idf + local4_idf
tf = local1_tf + local2_tf + local3_tf + local4_tf
...
for i in range(2,12):
	for j in range(300):
		...
		output+=','
	output+='\n'
with open("thread_Output.txt", "w") as text_file:
